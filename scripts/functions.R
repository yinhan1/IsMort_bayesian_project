...
plot_survival_function <- function(data){
  data %>% 
    ...
    theme_minimal() +
    labs(x = "Days", y = "Survival function", color = "", linetype = "")
}
...
#### EXTRA: age 14 aco vs co --------------------------- ####

log_shape_sampler14 <- function(log_alpha, log_theta, multiplier){
  m = matrix(c(0.0003549801,-0.0008649995,-0.0008649995,0.0029837907), ncol=2)
  MASS::mvrnorm(1, c(log_alpha, log_theta), multiplier*m)
}

#### EXTRA: random sampler for coefficients ACO ------------------------------- ####

coefs_sampler14 <- function(coefs, multiplier){
  m = matrix(
    c(
       0.0015469627 ,-0.0001946662 ,-0.0003274883 ,-0.0003725344 , 0.0005037472 , 0.0003126369,  0.0003746651, -0.0005850501,
      -0.0001946662,  0.0009166958,  0.0005115158,  0.0004361312, -0.0008358317, -0.0007971371, -0.0003858360,  0.0007126685,
      -0.0003274883,  0.0005115158,  0.0007642982,  0.0004482514, -0.0007770057, -0.0004209217, -0.0007080075,  0.0006934193,
      -0.0003725344,  0.0004361312,  0.0004482514,  0.0007791891, -0.0004009547, -0.0007030489, -0.0007287859,  0.0006395838,
       0.0005037472, -0.0008358317, -0.0007770057, -0.0004009547,  0.0014319145,  0.0008343866,  0.0008099863, -0.0014987915,
       0.0003126369, -0.0007971371, -0.0004209217, -0.0007030489,  0.0008343866,  0.0012841470,  0.0007007397, -0.0013149069,
       0.0003746651, -0.0003858360, -0.0007080075, -0.0007287859,  0.0008099863,  0.0007007397,  0.0012775349, -0.0013005410,
      -0.0005850501,  0.0007126685,  0.0006934193,  0.0006395838, -0.0014987915, -0.0013149069,  -0.0013005410, 0.0024929476
    ),
    ncol = length(coefs)
  )
  MASS::mvrnorm(1, coefs, multiplier*m)
...
#### EXTRA: age 28 aco vs co --------------------------- ####

log_shape_sampler28 <- function(log_alpha, log_theta, multiplier){
  m = matrix(c(0.0009006298,-0.0017543116,-0.001754312,0.003954001), ncol=2)
  MASS::mvrnorm(1, c(log_alpha, log_theta), multiplier*m)
}

#### EXTRA: random sampler for coefficients ACO ------------------------------- ####

coefs_sampler28 <- function(coefs, multiplier){
  m = matrix(
    c(
      0.0015469627 ,-0.0001946662 ,-0.0003274883 ,-0.0003725344 , 0.0005037472 , 0.0003126369,  0.0003746651, -0.0005850501,
      -0.0001946662,  0.0009166958,  0.0005115158,  0.0004361312, -0.0008358317, -0.0007971371, -0.0003858360,  0.0007126685,
...
#### EXTRA: age 42 aco vs co --------------------------- ####

log_shape_sampler42 <- function(log_alpha, log_theta, multiplier){
  m = matrix(c(0.001876819,-0.003009189,-0.003009189,0.006957252), ncol=2)
  MASS::mvrnorm(1, c(log_alpha, log_theta), multiplier*m)
}

#### EXTRA: random sampler for coefficients ACO ------------------------------- ####

coefs_sampler42 <- function(coefs, multiplier){
  m = matrix(
    c(
       0.0025470396, -0.0009808329, -0.0006786951, -0.0005796319,  0.0004100760,  0.0004330559,  0.0005960396, -0.0009884010,
      -0.0009808329,  0.0010202135,  0.0004723681,  0.0005682026, -0.0006824283, -0.0006536393, -0.0003313138,  0.0008917542,
      -0.0006786951,  0.0004723681,  0.0007796422,  0.0004055673, -0.0007082699, -0.0004087009, -0.0007520359,  0.0007193629,
      -0.0005796319,  0.0005682026,  0.0004055673,  0.0018283171, -0.0003193236, -0.0017716091, -0.0014622632,  0.0017540957,
       0.0004100760, -0.0006824283, -0.0007082699, -0.0003193236,  0.0013526233,  0.0006599356,  0.0007425339, -0.0011898933,
       0.0004330559, -0.0006536393, -0.0004087009, -0.0017716091,  0.0006599356,  0.0040241314,  0.0016085828, -0.0040430140,
       0.0005960396, -0.0003313138, -0.0007520359, -0.0014622632,  0.0007425339,  0.0016085828,  0.0034277169, -0.0034551109,
      -0.0009884010,  0.0008917542,  0.0007193629,  0.0017540957, -0.0011898933, -0.0040430140, -0.0034551109,  0.0075596746
    ),
    ncol = length(coefs)
  )
  MASS::mvrnorm(1, coefs, multiplier*m)
